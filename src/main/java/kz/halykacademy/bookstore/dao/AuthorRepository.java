...
public interface AuthorRepository extends JpaRepository<Author, Integer> {
    @Query(value = "select * from authors where name like %:name% or surname like %:name% or patronymicname like %:name% ", nativeQuery = true)
    ...
    @Query(value = "select distinct a.* from authors a " +
            "join book_author ba on a.id = ba.author_id " +
            "join book_genre bg on ba.book_id = bg.book_id " +
